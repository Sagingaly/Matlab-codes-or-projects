...
road_width = 100; % Adjust as needed
...
for i = 2:size(coordinates, 1)-1
    ...
    % Calculate bearing and distance for turn
    [bearing1, dist1] = bearing(lat1, lon1, lat2, lon2);
    [bearing2, dist2] = bearing(lat2, lon2, lat3, lon3);

    % Adjust the vehicle path to make the car follow the center
    ideal_trajectory = [ideal_trajectory; coordinates(i,:)];

    % Calculate left and right road boundaries by offsetting based on bearing
    left_bearing = bearing1 + pi/2; % Rotate 90 degrees to the left
    right_bearing = bearing1 - pi/2; % Rotate 90 degrees to the right

    % Calculate new positions for left and right boundaries
    left_road_point = [coordinates(i,1) + road_width/2 * cos(left_bearing), coordinates(i,2) + road_width/2 * sin(left_bearing)];
    right_road_point = [coordinates(i,1) - road_width/2 * cos(right_bearing), coordinates(i,2) - road_width/2 * sin(right_bearing)];

    left_road_trajectory = [left_road_trajectory; left_road_point];
    right_road_trajectory = [right_road_trajectory; right_road_point];

    % Update car marker position on the plot
    set(car_marker, 'XData', coordinates(i, 2), 'YData', coordinates(i, 1));
    
    % Pause for animation
    pause(0.05);
end

% Plot the final trajectories and road boundaries
plot(ideal_trajectory(:,2), ideal_trajectory(:,1), 'g', 'LineWidth', 2); % Ideal line in green
plot(left_road_trajectory(:,2), left_road_trajectory(:,1), 'r--', 'LineWidth', 2); % Left road boundary
plot(right_road_trajectory(:,2), right_road_trajectory(:,1), 'r--', 'LineWidth', 2); % Right road boundary
